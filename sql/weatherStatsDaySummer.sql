...
FROM (select @rownum := 0) as r
  ...
  left join (select Station
       ...
       --    and hour(il.When) = 14
           ...
           and ((month(il.When) * 100) + day(il.When) > 0430 and
                (month(il.When) * 100) + day(il.When) < 1101)
 ...
 where Temperature between -40 and 130
   ...
   and ((month(ol.When) * 100) + day(ol.When) > 0430 and
        (month(ol.When) * 100) + day(ol.When) < 1101)
